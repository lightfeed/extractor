...
describe("safeSanitizedParser", () => {
  ...
  describe("Nullable Schemas", () => {
    ...
    test("should handle valid non-null values in nullable fields", () => {
      const schema = z.object({
        name: z.string(),
        email: z.string().email().nullable(),
      });
    ...
    test("should convert invalid values to null in nullable fields", () => {
      const schema = z.object({
        name: z.string(),
        email: z.string().email().nullable(),
    ...
    test("should handle nullable arrays and objects", () => {
      const schema = z.object({
        name: z.string(),
        metadata: z
          .object({
    ...
    test("should handle invalid nested nullable objects", () => {
      const schema = z.object({
        user: z.object({
          name: z.string(),
          profile: z
      ...
      const expected = {
        user: {
          name: "Test User",
          profile: null, // Converted to null because of invalid website
        },
      };
...
describe("transformSchemaForLLM", () => {
  ...
  test("should handle nullable schemas", () => {
    ...
    const transformed = transformSchemaForLLM(schema);

    // Verify the structure is preserved
    expect(transformed).toBeInstanceOf(z.ZodObject);

    // Verify website field is transformed and still nullable
    const websiteField = (transformed as any).shape.website;
    expect(websiteField).toBeInstanceOf(z.ZodNullable);

    // Verify the inner type is a string without URL validation
    const innerWebsiteType = websiteField.unwrap();
    expect(innerWebsiteType).toBeInstanceOf(z.ZodString);
    expect(
      (innerWebsiteType as any)._def.checks.some((c: any) => c.kind === "url")
    ).toBeFalsy();

    // Verify profile field description is preserved
    const profileField = (transformed as any).shape.profile;
    expect(profileField).toBeInstanceOf(z.ZodNullable);
    expect((profileField as any)._def.description).toBe("User profile URL");
  });
});
